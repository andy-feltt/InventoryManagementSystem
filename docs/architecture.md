...
Clean Architecture keeps business rules independent from frameworks and infrastructure. The inventory rules can be tested without ASP.NET Core, PostgreSQL or Entity Framework, while the API and database layers can evolve with less risk.
...
- **Infrastructure** contains PostgreSQL persistence, `DbContext`, repositories, password hashing and database seed logic.
...
6. Infrastructure repositories persist through EF Core and PostgreSQL.
